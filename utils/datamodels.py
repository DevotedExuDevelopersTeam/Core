...
class Database:
    ...
    async def get_member_afk(self, id: int) -> tuple[str, datetime] | tuple[None, None]:
        r = await self.execute("SELECT afk FROM afks WHERE id = $1", id, fetch_mode=FetchMode.ROW)
        if r is None:
            return None, None
    ...
    async def set_member_afk(self, id: int, afk: str):
        await self.execute("INSERT INTO afks (id, afk) VALUES ($1, $2) ON CONFLICT (id) DO UPDATE SET afk = $2", id, afk)

    async def add_temprole(self, user_id: int, role_id: int, duration: timedelta):
        await self.execute("INSERT INTO temproles (id, role_id, remove_at) VALUES ($1, $2, $3)", user_id, role_id, datetime.now() + duration)

    async def add_warn(self, target_id: int, issuer_id: int, rule_violated: str):
        await self.execute("INSERT INTO warns (target_id, issuer_id, rule_violated) VALUES ($1, $2, $3)", target_id, issuer_id, rule_violated)

    async def delete_warn(self, id: int) -> bool:
        """Returns True if warning existed, False if it didn't"""
        return await self.execute("WITH deleted AS (DELETE FROM warns WHERE id = $1 RETURNING *) SELECT COUNT(*) > 0 FROM deleted", id, fetch_mode=FetchMode.VAL)

    async def clear_warns(self, user_id: int) -> int:
        """Returns amount of warnings cleared"""
        return await self.execute("WITH deleted AS (DELETE FROM warns WHERE target_id = $1 RETURNING *) SELECT COUNT(*) FROM deleted", user_id, fetch_mode=FetchMode.VAL)

    async def get_warns(self, user_id: int) -> list[Warn]:
        return [Warn(*r.values()) for r in await self.execute(
            "SELECT (id, target_id, issuer_id, issued_at, rule_violated) \
FROM warns WHERE target_id = $1", user_id, fetch_mode=FetchMode.ALL)]

    async def add_locked_channel(self, channel_id: int, duration: timedelta):
        await self.execute("INSERT INTO locked_channels (id, unlock_at) VALUES ($1, $2)", channel_id, datetime.now() + duration)
    ...
    async def get_level_roles(self) -> dict[int, int]:
        ...
        if len(self._cache.level_roles) == 0:
            self._cache.level_roles = {r["required_score"]: r["role_id"] for r in await self.execute("SELECT required_score, role_id FROM levels ORDER BY required_score", fetch_mode=FetchMode.ALL)}
        return self._cache.level_roles

    async def get_users_score(self, user_id: int) -> int:
        return await self.execute("SELECT score_total FROM scores WHERE id = $1", user_id, fetch_mode=FetchMode.VAL) or 0

    async def update_users_score(self, user_id: int, delta: int) -> int:
        await self.execute("INSERT INTO scores (id, score_total) VALUES ($1, $2) ON CONFLICT (id) DO UPDATE SET score_total = scores.score_total + $2", user_id, delta)

    async def get_lb_position(self, score: int) -> int:
        return await self.execute("SELECT COUNT(*) + 1 FROM scores WHERE score_total > $1", score, fetch_mode=FetchMode.VAL)

    async def get_top_data(self, page: int):
        return await self.execute("SELECT id, score_total FROM scores ORDER BY score_total LIMIT 10 OFFSET $1", (page - 1) * 10, fetch_mode=FetchMode.ALL)

    async def add_level(self, role_id: int, required_score: int):
        await self.execute("INSERT INTO levels (required_score, role_id) VALUES ($1, $2)", required_score, role_id)
    ...
    async def remove_level(self, value: disnake.Role | int) -> tuple[int, int] | tuple[None, None]:
        ...
        if isinstance(value, disnake.Role):
            r = await self.execute("DELETE FROM levels WHERE role_id = $1 RETURNING *", value.id, fetch_mode=FetchMode.ROW)
        elif isinstance(value, int):
            r = await self.execute("DELETE FROM levels WHERE required_score = $1 RETURNING *", value,
    ...
    async def get_youtubers(self) -> list[Youtuber]:
        return [Youtuber(*d.values()) for d in await self.execute("SELECT id, youtube_id, last_video FROM youtubers", fetch_mode=FetchMode.ALL)]
    ...
    async def add_youtuber(self, id: int, youtube_id: str, premium: bool):
        ...
        await self.execute("INSERT INTO youtubers (id, youtube_id, last_video, is_premium) VALUES ($1, $2, $3, $4)", id, youtube_id, last_video, premium)
    ...
    async def get_button_roles(self) -> dict[str, int]:
        return {r["id"]: r["role_id"] for r in await self.execute("SELECT (id, role_id) FROM button_roles", fetch_mode=FetchMode.ALL)}

    async def add_button_role(self, id: str, role_id: int, message_id: int):
        await self.execute("INSERT INTO button_roles (id, role_id, message_id) VALUES ($1, $2, $3)", id, role_id, message_id)

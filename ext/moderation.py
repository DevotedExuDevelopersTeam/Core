...
from utils.converters import TimeConverter, RuleConverter
...
class ModerationCommands(Cog):
    ...
    async def mute(self, inter: disnake.ApplicationCommandInteraction, user: disnake.Member, time: TimeConverter, rule: RuleConverter = RulesAutocomplete):
        if user.top_role >= inter.user.top_role:
            raise HierarchyError()

        await user.timeout(duration=time, reason=f"Mod: {inter.user} | Rule: {rule.id}")

        await inter.send(f"{user.mention} was timeouted for `{time}`. Violated rule: `{rule}`")
    ...
    async def filemute(self, inter: disnake.ApplicationCommandInteraction, user: disnake.Member, time: TimeConverter):
        if user.top_role >= inter.user.top_role:
            raise HierarchyError()
    ...
    async def ban(self, inter: disnake.ApplicationCommandInteraction, user: disnake.Member, rule: RuleConverter = RulesAutocomplete, time: TimeConverter = None, delete_message_days: int = commands.Param(0, ge=0, le=7)):
        if user.top_role >= inter.user.top_role:
            raise HierarchyError()
        ...
        await user.ban(delete_message_days=delete_message_days, reason=f"Mod: {inter.user} | Duration: {time} | Rule: {rule.id}")
        await inter.send(f"**{user}** was banned {time_str} for breaking rule `{rule.id}`")
    ...
    async def kick(self, inter: disnake.ApplicationCommandInteraction, user: disnake.Member, rule: RuleConverter = RulesAutocomplete):
        if user.top_role >= inter.user.top_role:
            raise HierarchyError()
...
class WarningsManagement(Cog):
    ...
    async def warn(self, inter: disnake.ApplicationCommandInteraction, user: disnake.Member, rule: RuleConverter = RulesAutocomplete):
        if user.top_role >= inter.user.top_role:
            raise HierarchyError()

        await self.bot.db.add_warn(user.id, inter.user.id, rule.id)
        total_warns = await self.bot.db.execute("SELECT COUNT(*) FROM warns WHERE target_id = $1", user.id, fetch_mode=FetchMode.VAL)
        warns_for_rule = await self.bot.db.execute("SELECT COUNT(*) FROM warns WHERE target_id = $1 AND rule_violated = $2", user.id, rule.id, fetch_mode=FetchMode.VAL)
        await inter.send(f"Successfully assigned warning to {user.mention}. Violated rule: `{rule}`\n\
This user now has `{total_warns}` warnings totally and `{warns_for_rule}` warnings for rule `{rule.id}`.")
        ...
        try:
            await user.send(f"You were warned in {inter.guild.name}. \
...
Violated rule: `{warning.rule_violated}`")
    ...
    async def delwarns(self, inter: disnake.ApplicationCommandInteraction, user: disnake.Member):
        ...
        if count > 0:
            await inter.send(f"Successfully removed **{count}** warning{s_(count)} from {user.mention}")
        else:
...
class ChannelsModeration(Cog):
    ...
    async def lock(self, inter: disnake.ApplicationCommandInteraction, channel: disnake.TextChannel = None, time: TimeConverter = None):
        ...
        if not user_perms.send_messages or not user_perms.read_messages:
            await inter.send("You do not have right to lock that channel", ephemeral=True)
            return
        ...
        await channel.set_permissions(inter.guild.default_role, send_messages=False, reason=f"Mod: {inter.user}")
        if time is not None:
            await self.bot.db.add_locked_channel(channel.id, time)

        await inter.send(f"Locked channel {channel.mention}{' for ' + timedelta_to_full_str(time) if time is not None else ''}")
    ...
    async def unlock(self, inter: disnake.ApplicationCommandInteraction, channel: disnake.TextChannel = None):
        ...
        await channel.set_permissions(inter.guild.default_role, send_messages=None, reason=f"Mod: {inter.user}")
    ...
    async def slowmode(self, inter: disnake.ApplicationCommandInteraction, slowmode: TimeConverter, channel: disnake.TextChannel = None):
        channel = channel or inter.channel
        user_perms = channel.permissions_for(inter.user)
        if not user_perms.send_messages or not user_perms.read_messages:
            await inter.send("You do not have right to change slowmode in that channel", ephemeral=True)
            return

        await channel.edit(slowmode_delay=slowmode.seconds)
        await inter.send(f"Successfully set the slowmode in {channel.mention} to **{timedelta_to_full_str(slowmode)}**")

    @commands.slash_command(name="purge", description="Purges messages in a channel")
    async def purge(self, inter: disnake.ApplicationCommandInteraction, amount: int, member: disnake.Member = None, channel: disnake.TextChannel = None):
        channel = channel or inter.channel
        user_perms = channel.permissions_for(inter.user)
        if not user_perms.send_messages or not user_perms.read_messages:
            await inter.send("You do not have right to purge that channel", ephemeral=True)
            return
        ...
        await inter.send(f"Successfully purged **{purged}** messages in {channel.mention}", ephemeral=True)

    @commands.slash_command(name="purgetill", description="Purges all messages below the given")
    async def purgetill(self, inter: disnake.ApplicationCommandInteraction, message_id: int):
        ...
        await inter.send(f"Successfully purged **{purged}** messages in {inter.channel.mention}", ephemeral=True)
    ...
    async def lockserver(self, inter: disnake.ApplicationCommandInteraction):
        ...
        self.locked_channels = [c for c in inter.guild.text_channels if c.permissions_for(inter.guild.default_role).send_messages is not False]
        for c in self.locked_channels:
            await c.set_permissions(inter.guild.default_role, send_messages=False, reason=f"Server lock done by {inter.user}")
        await inter.send("Successfully locked the server. Please do not restart bot until server is unlocked")
    ...
    async def unlockserver(self, inter: disnake.ApplicationCommandInteraction):
        await inter.response.defer()
        for c in self.locked_channels:
            await c.set_permissions(inter.guild.default_role, send_messages=None, reason=f"Server unlock done by {inter.user}")

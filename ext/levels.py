...
from utils.constants import XP_COOLDOWN_SECONDS, XP_IGNORED_CHANNELS_IDS, XP_BOUNDS
from utils.errors import AdminOnly
from utils.utils import get_next_score, sep_num
from utils.image_generator import draw_rank_card, draw_leaderboard
...
class Levels(Cog):
    ...
    async def _check_level_roles(self, member: disnake.Member, channel: disnake.TextChannel):
        ...
        if len(roles_to_add) > 0:
            ...
            if top_role in roles_to_add:
                await channel.send(f"GG {member.mention}, you just earned <@&{top_role}>!", allowed_mentions=disnake.AllowedMentions.none())
    ...
    async def xp_controller(self, message: disnake.Message):
        if message.author.bot or message.channel.id in XP_IGNORED_CHANNELS_IDS or message.channel.category_id in XP_IGNORED_CHANNELS_IDS or self._is_member_on_cooldown(message.author.id):
            return
    ...
    async def rank(self, inter: disnake.ApplicationCommandInteraction, user: disnake.Member = None):
        ...
        await inter.send(file=disnake.File(await draw_rank_card(user, await self.bot.db.get_lb_position(current_score), next_role, current_score, next_score)))
...
class LevelsManagement(Cog):
    ...
    async def addlevel(self, inter: disnake.ApplicationCommandInteraction, role: disnake.Role, required_score: int):
        await self.bot.db.add_level(role.id, required_score)
        await inter.send(f"Successfully added level with role {role.mention} and score `{sep_num(required_score, '.')}`", allowed_mentions=disnake.AllowedMentions.none())
    ...
    async def removelevel_role(self, inter: disnake.ApplicationCommandInteraction, role: disnake.Role):
        ...
        if role_id is None:
            ...
            return
        await inter.send(f"Successfully removed level with role {role.mention} and score `{sep_num(required_score, '.')}`", allowed_mentions=disnake.AllowedMentions.none())
    ...
    async def removelevel_score(self, inter: disnake.ApplicationCommandInteraction, score: int):
        ...
        if role_id is None:
            ...
            return
        await inter.send(
            f"Successfully removed level with role <@&{role_id}> and score `{sep_num(required_score, '.')}`", allowed_mentions=disnake.AllowedMentions.none())

    @commands.slash_command(name="addscore", description="Adds score to a member")
    async def addscore(self, inter: disnake.ApplicationCommandInteraction, user: disnake.Member, score: int = commands.Param(ge=1, le=1_000_000)):
        await self.bot.db.update_users_score(user.id, score)
        await inter.send(f"Successfully added `{sep_num(score, ' ')}` score to {user.mention}")

    @commands.slash_command(name="removescore", description="Removes score from a member")
    async def removescore(self, inter: disnake.ApplicationCommandInteraction, user: disnake.Member, score: int = commands.Param(ge=1, le=1_000_000)):
        await self.bot.db.update_users_score(user.id, -score)
        await inter.send(f"Successfully removed `{sep_num(score, ' ')}` score from {user.mention}")

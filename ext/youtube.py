...
from utils.constants import YOUTUBE_CHANNEL_ID, NEW_VIDEOS_ROLE_ID, APPLICATIONS_CHANNEL_ID
    ...
    async def youtube_fetcher(self):
        ...
        for youtuber in youtubers:
            try:
                ...
                self.bot.log.error("Failed to fetch video for youtuber %s (%s)", youtuber.youtube_id, await self.bot.server.get_or_fetch_member(youtuber.id))
                continue
            if last_video != youtuber.last_video:
                ...
                if member is None:
                    ...
                    self.bot.log.warning("Youtuber %s left so was removed from content creators program", youtuber.id)
                    continue
...
class YoutubeManagement(Cog):
    ...
    async def addyt(self, inter: disnake.ApplicationCommandInteraction, user: disnake.Member, youtube_id: str, premium: bool):
        await self.bot.db.add_youtuber(user.id, youtube_id, premium)
        await inter.send(f"Successfully added {user.mention} to content creators program")
    ...
    async def removeyt(self, inter: disnake.ApplicationCommandInteraction, user: disnake.Member):
        ...
        if is_youtuber:
            await inter.send(f"Successfully removed {user.mention} from content creators program")
        else:
